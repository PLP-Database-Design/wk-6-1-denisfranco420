...
|---------|------------------|----------|-----------------|------------|--------|------------|-------------------|----------------|
| RISK-001 | Payment taken but order not created | Functional | Payments (FR-O03) | 3 | 5 | **15** | End-to-end payment testing with failure scenarios; verify database consistency | TC-041 |
| RISK-002 | XSS vulnerability via markdown `javascript:` links | Security | Reviews/Q&A (FR-U03) | 4 | 5 | **20** | Input sanitization testing; security penetration testing on all user inputs | TC-042|
| RISK-003 | User data isolation failure - one user sees another's data | Security | User Accounts | 3 | 5 | **15** | Multi-user session testing; authorization boundary testing | TC-043 |
| RISK-004 | Inventory race conditions leading to overselling | Technical | Cart/Inventory (FR-O01) | 4 | 4 | **16** | Load testing on cart operations; verify stock validation at multiple points | TC-007 |
...
| Risk ID | Risk Description | Category | Functional Area | Likelihood | Impact | Risk Score | Mitigation Strategy | Test case ID |
|---------|------------------|----------|-----------------|------------|--------|------------|-------------------|---------------| 
| RISK-005 | Financial calculation errors (tax, shipping, coupons) | Functional | Checkout (FR-O02) | 4 | 4 | **16** | Boundary value testing on calculations; verify rounding rules | TC-013<br>TC-014|
| RISK-006 | Order state corruption or invalid status transitions | Functional | Orders (FR-O05) | 3 | 4 | **12** | State transition testing; audit trail verification | TC-044 |
| RISK-007 | Authentication bypass - non-admin accessing admin functions | Security | Admin Console | 3 | 4 | **12** | Privilege escalation testing; localStorage manipulation tests | TC-045|
